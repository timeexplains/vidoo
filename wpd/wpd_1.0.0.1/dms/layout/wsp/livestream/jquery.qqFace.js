	...
	$.fn.qqFace = function(options){
		...
		$(this).click(
			function(e){
				...
				var strFace, labFace;
				if($('#'+id).length<=0){
					strFace = '<div id="'+id+'" style="background: #fff;position:absolute;display:none;z-index:99999;" class="qqFace">'; 
			        ...
			        strFace +='<div class="swiper-slide">'
			         +'<ul class="qq_face" >';
						for(var i=1; i<=21; i++){
							labFace = '['+tip+i+']';
							strFace += '<li><i src="" class="qqface'+i+'" onclick="$(\'#'+option.assign+'\').setCaret();$(\'#'+option.assign+'\').insertAtCaret(\'' + labFace + '\');" /></li>';
						}
					strFace += '</ul></div>';
			        strFace +='<div class="swiper-slide">'
			          +'<ul class="qq_face" >';
					...
					strFace += '</ul></div>';
		       		strFace += '</div></div><div class="pagination" ></div></div>';
				}
				$(this).parent().parent().parent().append(strFace);
				var fbwidth=$(window).width();
				var fbheight=$(window).height()/3;
				fbheight=115;
				//var tablecp=parseInt((fbwidth-31*7)/14);
				$(".swiper-container").css({"height":fbheight,"width":fbwidth});
				//$(".swiper-slide ul li").css("margin-right",tablecp);
